...
---------------------------------------------------------------------------
---------------------------------------------------------------------------
   Solves model for three levels of risk aversion: 
   - For sigma = 1-theta (benchmark), saves the results in results_riskaversion.mat
   - For sigma = 0 (neutral), saves the results in results_ra_0.mat 
   - For sigma = 1.5 (averse), saves the results in results_ra_1.5.mat

 For each level of risk aversion, solves for two elasticities of substitution
   and various levels of signal precision (behind Figure 9).
---------------------------------------------------------------------------
---------------------------------------------------------------------------
...
for ss = 1:3  % Solves model for each risk aversion
    
    if ss == 1
            disp('Benchmark risk aversion')
        elseif ss==2
            disp('Risk neutral')
        elseif ss==3
            disp('Risk averse')
    ...
    for l = 1:2 %Solves model for each elasticity of substitution
        ...
        elseif ss==2
            param.sigma = 0;                %Risk-neutral
        elseif ss==3
            param.sigma = 1.5;              %Risk-averse
        end
        ...
        %% Solve model for various levels of signal precision
        ...
        for j=1:N_eta   %For loop for each signal precision
            ...
            if l == 1
                Export_eta_r_1 = Exports_eta_r;
            end
        end
    end
    
    if ss == 1
        save results_ra_bench
    elseif ss==2
        save results_ra_neutral
    elseif ss==3
        save results_ra_averse
    end
